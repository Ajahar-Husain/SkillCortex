...
export default function InterviewSession() {
    ...
    const { job, resume, applicationId } = location.state || {};

    // Structured interview session (PRD §47 / §19) — the server owns the timer,
    // transcript and evaluation; this page is the realtime front-end for it.
    const interviewIdRef = useRef(null);
    const modeRef = useRef('legacy'); // 'structured' | 'legacy' fallback
    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = () => {
            if (document.hidden) {
                // PRD §24 — report the event to the server so it lands in the
                // integrity trail reviewed by HR (structured flow only).
                if (interviewIdRef.current && modeRef.current === 'structured') {
                    api.post(`/api/interviews/${interviewIdRef.current}/event`, { type: 'TAB_SWITCH', detail: 'Document hidden during interview' }).catch(() => {});
                }
                setWarnings(prev => {
    ...
    // PRD §23/§24/§62 — fullscreen-exit integrity events + session heartbeat.
    // The heartbeat restores a dropped connection and hands timer authority to
    // the server (§63); every failure degrades gracefully (§84).
    useEffect(() => {
        const postEvent = (type, detail) => {
            const id = interviewIdRef.current;
            if (!id || modeRef.current !== 'structured') return;
            api.post(`/api/interviews/${id}/event`, { type, detail }).catch(() => {});
        };
        const onFullscreenChange = () => {
            if (!document.fullscreenElement) postEvent('FULLSCREEN_EXIT', 'Left fullscreen during interview');
        };
        document.addEventListener('fullscreenchange', onFullscreenChange);
        const heartbeat = setInterval(async () => {
            const id = interviewIdRef.current;
            if (!id || modeRef.current !== 'structured' || interviewComplete) return;
            try {
                const { data } = await api.post(`/api/interviews/${id}/heartbeat`);
                if (data.expired) handleEndInterview('Time is up. Concluding the interview now...');
            } catch { /* network hiccup — keep the session usable */ }
        }, 20000);
        return () => {
    ...
    const generateAIResponse = async (history) => {
        ...
        try {
            const id = interviewIdRef.current;
            // Structured flow: the server generates the next adaptive question
            // (§64) and evaluates the previous answer (§25) in one round-trip.
            if (modeRef.current === 'structured' && id) {
                const lastUser = [...history].reverse().find((m) => m.role === 'user');
                if (lastUser) {
                    const res = await api.post(`/api/interviews/${id}/answer`, { answer: lastUser.content });
                    const aiResponse = res.data.nextQuestion;
    ...
    const startInterview = async () => {
        ...
        // PRD §83 — try the structured session first; on failure fall back to
        // the legacy chat flow so the apply loop never dead-ends (§84).
        try {
            ...
            setTimeLeft(data.durationMin ? data.durationMin * 60 : 600);
            const firstQ = data.firstQuestion || 'Introduce yourself and describe your strongest skill for this role.';
            setMessages([
                { role: 'system', content: `AI Interview started — ${data.durationMin || 10} minutes for the ${job.title} role.` },
                { role: 'ai', content: firstQ },
            ]);
            speakText(firstQ, () => {
                try {
                    recognitionRef.current?.start();
            ...
            console.warn('Structured start failed, using legacy chat flow', err);
    ...
    const handleEndInterview = async (finalMessage = "Concluding interview. Thank you for your time.") => {
        ...
        try {
            ...
            const id = interviewIdRef.current;
            // Structured flow (§27/§29/§31/§83): finish server-side, then hand
            // off to the coding task when the role requires one — the final
            // report is generated after the code submission.
            if (modeRef.current === 'structured' && id) {
                let handoff = null;
                if (job.codingConfig?.required) {
                    try {
                        const ct = await api.post(`/api/interviews/${id}/coding-task`);
                        handoff = ct.data?.assessment || null;
                    } catch (e) {
                        console.warn('Coding task provisioning failed', e);
                    }
                }
                if (!handoff) {
                    // No coding phase → run the final analysis now.
                    await api.post(`/api/interviews/${id}/complete`, {});
                    setMessages(prev => [...prev, { role: 'system', content: 'Evaluation Complete. Opening your report...' }]);
                    navigate(`/interview/${id}/result`);
                } else {
                    setMessages(prev => [...prev, { role: 'system', content: 'Verbal assessment complete. Moving to the coding challenge...' }]);
                    setTimeout(() => navigate(`/assessment/${handoff._id}/code`, { state: { interviewId: id, job } }), 1500);
                }
                return;
            }

            await api.post('/api/interview/evaluate', {
                jobId: job._id,
                transcript: messages
            });
